    ...
    : DialogBase(CFileManagerDlg::IDD, pParent, pIOCPServer, pContext, IDI_File)
{
    ...
    DRIVE_Sys = FALSE;
    DRIVE_CAZ = FALSE;
bool CFileManagerDlg::LoadDriveListFromContext()
{
    if (!m_ContextObject) {
        return false;
    }
    ...
    const ULONG bufferLen = m_ContextObject->m_DeCompressionBuffer.GetBufferLen();
    if (bufferLen < 3) {
        return false;
    }

    if (m_ContextObject->m_DeCompressionBuffer.GetBYTE(0) != TOKEN_DRIVE_LIST) {
        return false;
    }

    LPBYTE adminPtr = m_ContextObject->m_DeCompressionBuffer.GetBuffer(1);
    LPBYTE pathPtr = m_ContextObject->m_DeCompressionBuffer.GetBuffer(2);
    if (!adminPtr || !pathPtr) {
        return false;
    }

    ULONG bytesRemaining = bufferLen - 2;
    ULONG pathBytes = 0;
    bool foundTerminator = false;
    while (pathBytes + sizeof(TCHAR) <= bytesRemaining) {
        if (*reinterpret_cast<TCHAR*>(pathPtr + pathBytes) == _T('\0')) {
            foundTerminator = true;
            break;
        }
        pathBytes += sizeof(TCHAR);
    }
    ...
    m_bCanAdmin = (*adminPtr) == 0x01;
    m_strDesktopPath = reinterpret_cast<TCHAR*>(pathPtr);

    ULONG driveOffset = 1 + 1 + pathBytes + sizeof(TCHAR);
    if (bufferLen <= driveOffset) {
        return false;
    }

    ULONG driveLength = std::min<ULONG>(bufferLen - driveOffset, sizeof(m_bRemoteDriveList) - 1);
    memset(m_bRemoteDriveList, 0, sizeof(m_bRemoteDriveList));
    memcpy(m_bRemoteDriveList, m_ContextObject->m_DeCompressionBuffer.GetBuffer(driveOffset), driveLength);
    m_bRemoteDriveList[driveLength] = 0;
    return true;
}

void CFileManagerDlg::ApplyDriveListFromContext()
{
    if (LoadDriveListFromContext()) {
